export class InputHandler {
  ...
  handleMouseMove(e) {
    const { x, y } = this.getCanvasCoordinates(e.clientX, e.clientY);
    this.mousePos.x = x; this.mousePos.y = y;
    if (this.isDragging && this.selectedPiece) this.updatePreview(); else this.game.render();
  }
  ...
  handleMouseUp(e) {
    ...
    this.selectedPiece = null; this.selectedPieceIndex = null; this.isDragging = false; this.isTouch = false; this.game.render();
  }
  ...
  handleTouchStart(e) {
    ...
    if (pieceIndex !== null && !this.game.isGameOver && !this.game.isAnimating) {
      this.selectedPiece = this.game.hand[pieceIndex];
      this.selectedPieceIndex = pieceIndex;
      this.isDragging = true; this.isTouch = true; this.mousePos = { x, y };
    }
  }
  ...
  handleTouchMove(e) {
    ...
    const touch = e.touches[0];
    const { x, y } = this.getCanvasCoordinates(touch.clientX, touch.clientY);
    this.mousePos.x = x; this.mousePos.y = y; this.updatePreview();
  }
  ...
  getPieceAtPosition(x, y) {
    ...
    for (let i = 0; i < this.game.hand.length; i++) {
      const piece = this.game.hand[i]; if (!piece) continue;
      const slotX = startX + i * (pieceSlotWidth + spacingBetween);
      const pieceWidth = piece.width * handCellSize; const pieceHeight = piece.height * handCellSize;
      ...
      if (x >= pieceLeft && x <= pieceRight && y >= pieceTop && y <= pieceBottom) return i;
    }
    return null;
  }

  getGridPosition(canvasX, canvasY, piece = null) {
    let adjustedX = canvasX; let adjustedY = canvasY;
    if (piece) {
      ...
      adjustedX = canvasX - offsetX; adjustedY = canvasY - offsetY;
    }
    const gridX = Math.round((adjustedX - this.game.renderer.gridOffset.x) / this.game.renderer.cellSize);
    const gridY = Math.round((adjustedY - this.game.renderer.gridOffset.y) / this.game.renderer.cellSize);
    if (gridX < -10 || gridX >= 20 || gridY < -10 || gridY >= 20) return null;
  ...
  drawDragPreview() {
    ...
    if (gridPos) {
      const canPlace = this.game.grid.canPlacePiece(this.selectedPiece, gridPos.x, gridPos.y);
      if (canPlace) {
        const potentialClears = this.calculatePotentialClears(this.selectedPiece, gridPos.x, gridPos.y);
        if (potentialClears) this.game.renderer.highlightClears(potentialClears);
  ...
  calculatePotentialClears(piece, startX, startY) {
    ...
    for (let row = 0; row < piece.shape.length; row++) {
      for (let col = 0; col < piece.shape[row].length; col++) {
        if (piece.shape[row][col] === 1) tempCells[startY + row][startX + col] = 1;
      }
    }
